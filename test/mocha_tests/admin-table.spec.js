...
import AdminTable from '../../src/components/displayTable/admin-table.js';
...
describe("Admin Table", () => {
  ...
  it("Should render the table when the values are passed to the component", () => {
    ...
    expect(wrapper.findAll("#fifthTable th").at(0).text()).to.equal(columns[0].displayName);
    expect(wrapper.findAll("#fifthTable td").at(0).text()).to.equal(rows[0][columns[0].propName]);
  });
